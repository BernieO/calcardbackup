...
- *optional*: das Paket `curl`, wenn die nicht empfohlene Option `-g|--geht-via-http` gesetzt wird
...
-c | --configfile DATEI
       ...
       Alle anderen angegebenen Optionen mit Ausnahme von '-b|--batch' werden ignoriert!
-d | --date FORMAT
       Benutze FORMAT als Dateinamenserweiterung für die komprimierte Backupdatei oder das Backupverzeichnis.
       FORMAT muss eine gültige Formatbeschreibung des Befehls date() sein.
...
-u | —usersfile DATEI
       ...
       Da die Option '-f|--fetch-from-database' das Standardverhalten von calcardbackup >= 0.8.0 ist, besteht keine
       Notwendigkeit mehr in dieser Datei Passwörter anzugeben oder diese Datei überhaupt zu benutzen.
       ACHTUNG: diese Option ist zwingend erforderlich, falls das Skript mit Option '-g|--get-via-http' aufgerufen wird.
-x | --uncompressed
...
Sichere keine Kalender (`-nc`) und speichere die Sicherung unkomprimiert (`-x`) im Verzeichnis `calcardbackup-YYYY-MM-DD` (standard) in `./backups/` (standard).
...
Ja, allerdings!  
*calcardbackup* benötigt lediglich die Datenbank (und Zugang zu ihr) einer ownCloud/Nextcloud installation, um Kalender/Adressbücher von der Datenbank auszulesen und sie als .ics und .vcf Datein zu sichern. Gehen Sie folgendermaßen vor:
